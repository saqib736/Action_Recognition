...
class ConvLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.lstm(x)
        print(f'before atten: {x.shape}')
        if self.attention:
            attention_w = F.softmax(self.attention_layer(x).squeeze(-1), dim=-1)
            x = torch.sum(attention_w.unsqueeze(-1) * x, dim=1)
        else:
            x = x[:, -1]
        print(f'before outputlayer: {x.shape}')
...
def main():
    ...
    # summary(model, input_size=(8, 30, 27, 192, 256))
    random_input1 = torch.randn(8, 30, 27, 192, 256)
    outputs = model(random_input1)
    # dot = make_dot(outputs, params=dict(list(model.named_parameters())))
    # dot.render('model_visualization', format='png', quiet=False)
    # dot.view()
